...
| **editor** | Owns the editor slot. Stack: selection (shift+arrows, ctrl+c/v/x, ctrl+shift+a, shift+del), history (↑ this session, shift+↑/ctrl+r = fuzzy menu over all sessions, rendered as pi's own autocomplete list so the input box stays live), `:q` shutdown, unframe (pi's rules, box caps and side rails dropped for a plain two-column indent), gantt board integration. The input box paints **nothing** — the old `▌` left bar sat inside the terminal's own text and got copied along with any selected prompt; the editor publishes its `borderColor` on `globalThis.__oilrigModePaint` and chrome paints the footer's mode bar with it instead. |
| **chrome** | Suppresses open-tui welcome header, wraps the footer, puts the model + thinking level in an `aboveEditor` widget. Footer line 1 is the mode bar: a full-width thick rule (`▀`) in the editor's live borderColor, replacing the old thin `─`. Every line below it is one left/right pair, ordered by importance: version tag + cwd against the context bar, then the extension statuses two per line by rank. No borders, no side rails, no greedy packing. |
...
| **starship** | Session telemetry — duration, turns, TPS, TTFT, tokens, stalls. Not a widget of its own: it is the first **web** card (one line in the strip, a broken-out card with the in/out split). The info surface is the per-cwd local website (`oilrig-web`), not an in-pi panel — see that package. |
...
The info surface is a per-cwd local website, owned by the **`oilrig-web`** package
...
| `f2` | Open the oilrig-web board in a browser |
...
Then:
```
pi install git:github.com/yesitsfebreeze/oilrig-ui
```
